...
router.get("/questions/counts", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT unit_index, lesson_index, COUNT(*)::int AS count
       FROM questions WHERE grade_key = $1
       GROUP BY unit_index, lesson_index`,
      [gradeKey]
    );
    const counts: Record<string, number> = {};
    result.rows.forEach((r) => {
      counts[`${r.unit_index}|${r.lesson_index}`] = r.count;
    });
    res.json(counts);
  } catch (e) {
...
router.get("/questions", async (req, res) => {
  try {
    ...
    if (gradeKey) {
      conditions.push(`grade_key = $${params.length + 1}`);
      params.push(gradeKey as string);
    }
    if (unitIndex !== undefined) {
      conditions.push(`unit_index = $${params.length + 1}`);
      params.push(Number(unitIndex));
    }
    if (lessonIndex !== undefined) {
      conditions.push(`lesson_index = $${params.length + 1}`);
      params.push(Number(lessonIndex));
    }
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO questions (grade_key, unit_index, lesson_index, question_text, option_a, option_b, option_c, option_d, correct_answer, image_url)
       ...
       ON CONFLICT (grade_key, unit_index, lesson_index, question_text) DO NOTHING
       RETURNING *`,
      [grade_key, unit_index, lesson_index, question_text, option_a, option_b, option_c, option_d, correct_answer, image_url || null]
    );
    if (result.rows.length === 0) {
      return res.status(409).json({ error: "السؤال موجود مسبقاً في هذا الدرس" });
    }
...
router.put("/questions/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE questions SET question_text=$1, option_a=$2, option_b=$3, option_c=$4, option_d=$5, correct_answer=$6, image_url=$7
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (!q.question_text || !q.option_a || !q.option_b || !q.option_c || !q.option_d || q.correct_answer === undefined) {
        errors.push({ row: i + 2, error: "بيانات ناقصة" });
        continue;
      }
      try {
